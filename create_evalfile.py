...
from codes_for_kaist.datasets_fusion import KaistPDDataset
...
def detect(img_id, image_lwir, image, min_score, max_overlap, top_k):
    """
    ...
    # for id, boxes in zip(img_id, det_boxes):
    ...
    # with open(os.path.join('./predictions_lwir_80_grayscale_stack.json'), 'w') as j: 
...
if __name__ == '__main__':
    ...
    for i, (image, image_lwir, boxes, labels, difficulties, image_id) in enumerate(tqdm(test_loader)):
        detect(image_id, image_lwir, image, min_score=0.2, max_overlap=0.5, top_k=200)
    
    
    os.makedirs('./eval_files/fusion', exist_ok=True)
    with open('./eval_files/fusion/80_halfway.json', 'w') as j:
